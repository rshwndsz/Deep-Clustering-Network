...
class DCN(nn.Module):
    ...
    def pretrain(self, train_loader, epoch=100, run=None, verbose=True):
        if not self.args.pretrain:
            return
        if verbose:
            print(self.autoencoder.encoder.__repr__())
        ...
        for e in range(epoch):
            for batch_idx, data in enumerate(train_loader):
                ...
                if verbose and batch_idx % self.args.log_interval == 0:
                    ...
                    if run:
                        run.log({"pre_train/rec_loss": rec_loss_list[-1]})
    ...
    def fit(self, epoch, train_loader, run=None, verbose=True):
        for batch_idx, data in enumerate(train_loader):
            batch_size = data.size()[0]
            ...
            if verbose and batch_idx % self.args.log_interval == 0:
                ...
                if run:
                    run.log(
                        {
                            "fit/loss": loss.detach().cpu().numpy(),
                            "fit/rec_loss": rec_loss,
                            "fit/dist_loss": dist_loss,
                        }
                    )
